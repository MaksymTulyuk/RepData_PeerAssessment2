...
casualties1[,c(2:4)] <- casualties1[,c(2:4)] / 1000000000
...
```{r, echo = FALSE}
ggplot(casualties1, aes(x = reorder(EventType, Casualties), y = Fatalities + Injuries)) + coord_flip() + geom_bar(stat="identity") + xlab("Event Types") + ylab("Casualties") + ggtitle("Impact on population")
```

### Impact on economy

The most harmful event on economy is `r damage1[1,1]` with the total damage `r damage1[1,4]` billions US dollars (the property damage `r damage1[1,2]` billions US dollars and the crop damage `r damage1[1,3]` billions US dollars)


```{r, echo = FALSE}
ggplot(damage1, aes(x = reorder(EventType, Damage), y = PropertyDamage + CropDamage)) + coord_flip() + geom_bar(stat="identity") + xlab("Event Types") + ylab("Total Damage (Billions USD)") + ggtitle("Impact on economy")
```
